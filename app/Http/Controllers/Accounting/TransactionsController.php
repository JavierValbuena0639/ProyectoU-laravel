...
class TransactionsController extends Controller
{
    ...
    /**
     * Cancel transaction (soft) ensuring domain consistency.
     */
    public function cancel(Request $request, Transaction $transaction)
    {
        ...
        // Ensure transaction belongs to user's domain via its account
        $domain = method_exists($user, 'emailDomain') ? $user->emailDomain() : ($user->email_domain ?? null);
        if (!$transaction->account || $transaction->account->service_domain !== $domain) {
            return redirect()->route('accounting.transactions')
                ->withErrors(['cancel' => 'No puede cancelar transacciones fuera de su dominio']);
        }
